...
int countTiles(char board[][26], int n, char playColour);

int minimax(char board[][26], int depth, bool computerPlaying,  char playColour, int n, char initalBoard[][26]);
...
int computerPlays(char board[][26], int n, char playColour, char movesComputer[26][3]){
    ...
    int value = makeMove(board, n, playColour, &row, &col);

     char letterRow = convertNum(row, board, n);
     char letterCol = convertNum(col, board, n);
    printf("Computer places %c at %c%c.\n", playColour, letterRow, letterCol);
    changeBoard(playColour, letterRow, letterCol, board, n);
    return 1;
...
int minimax(char board[][26], int depth, bool computerPlaying, char playColour, int n, char initalBoard[][26]){
    ...
    if (depth == 0){
        return countTiles(board, n, playColour);
    }

    if(computerPlaying){
        int bestScore = -1000; 
        char moves[26][3];
        int move = avaliableMoves(board, n, playColour, moves);
        for (int i = 0; i < move; i++){
            int newRow = moves[i][0];
            int newCol = moves[i][1];
            if (board[newRow][newCol] == 'U'){
                changeBoard(playColour, newRow, newCol, board, n);
                int score = minimax(board, depth-1, false, playColour, n, initalBoard);
                board = initalBoard;
                if (score > bestScore){
                    bestScore = score;
                }
        ...
        int bestScore = 1000; 
        char moves[26][3];
        int move = avaliableMoves(board, n, oppositeColour, moves);
        for (int i = 0; i < move; i++){
            int newRow = moves[i][0];
            int newCol = moves[i][1];
            if (board[newRow][newCol] == 'U'){
                changeBoard(oppositeColour, newRow, newCol, board, n);
                int score = minimax(board, depth-1, false, playColour, n, initalBoard);
                board = initalBoard;
                if (score < bestScore){
                    bestScore = score;
...
int makeMove(char board[26][26], int n, char turn, int *row, int *col){
    ...
    int move = avaliableMoves(board, n, turn, moves);

    for (int i = 0; i < move; i++){
        char charRow = moves[i][0];
        char charCol = moves[i][1];

        int newRow = convertRow(charRow, board);
        int newCol = convertRow(charCol, board);

        if(board[newRow][newCol] == 'U'){
            changeBoard(turn, charRow, charCol, board, n);
            int score = minimax(board, 3, false, turn, n, initialBoard);
            board = initialBoard;
            if(score > bestScore){
                bestScore = score; 
                bestRow = newRow;
